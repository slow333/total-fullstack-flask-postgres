...
function PsqlFunctions() {
  return (
    <div>
      <div id='psql-functions' className="for-space"></div>
      <h1>문자 관련 함수</h1>

      <h3>trim</h3>
      <pre>SELECT TRIM('  Hello World!  '); -- 'Hello World!'<br/>
      SELECT TRIM(LEADING ' ' FROM '  Hello World!  '); -- 'Hello World!  ' <br/>
      SELECT TRIM(TRAILING '*' FROM '  Hello World!****'); -- '  Hello World!'</pre>  

      <h3>concat</h3>
      <pre>SELECT CONCAT('Hello', ' ', 'World!'); -- 'Hello World!'<br/>
      ...
      <pre>SELECT INSTR('Hello World!', 'World'); -- 7 (찾은 문자열의 시작 위치)<br/>

      SELECT LOCATE('World', 'Hello World!'); -- 7 (찾은 문자열의 시작 위치)<br/>
      SELECT LOCATE('World', 'Hello World!', 8); -- 0 (8번째 이후에 찾지 못함)</pre>

      <h3>upper, lower</h3>
      <pre>SELECT UPPER('Hello World!'); -- 'HELLO WORLD!'<br/>
      ...
      <pre>SELECT LPAD('Hello', 10, '*'); -- '*****Hello'<br/>
      SELECT RPAD('Hello', 10, '*'); -- 'Hello*****'</pre>

      <h3>coalesce</h3>
      <pre>SELECT COALESCE(NULL, 'Hello', 'World'); -- 'Hello'<br/>
      SELECT COALESCE(NULL, NULL, 'World'); -- 'World'<br/>
      ...
      <pre>SELECT CASE WHEN 1 &gt; 0 THEN 'True' ELSE 'False' END; -- 'True'<br/>
      SELECT CASE <br/>
        WHEN 가격 &gt; 30 THEN '비싸요' <br/>
        WHEN 가격 &lt;= 30 THEN '저렴해요' <br/>
        ELSE '가격 정보 없음'<br/>
      END;</pre>

      <h3>ifnull; null 일때 기본값 지정</h3>
      <pre>SELECT IFNULL(NULL, 'Hello'); -- 'Hello'<br/>
      SELECT IFNULL('Hello', 'World'); -- 'Hello'</pre>

      <h3>nullif: 2개의 값이 같으면 null, 아니면 첫번째 값</h3>
      <pre>SELECT NULLIF('Hello', 'Hello'); -- NULL<br/>
      ...
      <h3>round; 숫자를 반올림</h3>
      <pre>SELECT ROUND(123.4567, 2); -- 123.46<br/>
      SELECT ROUND(123.4567); -- 123<br/>
      FLOOR(i + RAND() * (J - I); -- I와 J 사이의 정수<br/>
      update car set car.customer_id = FLOOR(rand()*10 + 1); -- 1~ 10의 값</pre>

      <h3>rand; 랜덤 숫자 생성</h3>
      <pre>SELECT RAND(); -- 0과 1 사이의 랜덤 숫자<br/>
      ...
      <h3>ceil; 숫자를 올림</h3>
      <pre>SELECT CEIL(123.4567); -- 124<br/>
      ...
      <h3>mod; 나머지 연산</h3>
      <pre>SELECT MOD(10, 3); -- 1<br/>
      SELECT 7 $ 2; -- 1</pre>

      <h3>abs; 절대값</h3>
      <pre>SELECT ABS(-123.4567); -- 123.4567<br/>
      SELECT ABS(123.4567); -- 123.4567</pre>

      <h3>sign; 숫자의 부호 반환</h3>
      <pre>SELECT SIGN(-123.4567); -- -1<br/>
      SELECT SIGN(0); -- 0<br/>
      SELECT SIGN(123.4567); -- 1</pre>

      <h3>greatest, least</h3>
      <pre>SELECT GREATEST(1, 2, 3); -- 3<br/>
      SELECT LEAST(1, 2, 3); -- 1</pre>

      <h3>convert: cast type</h3>
      <pre>SELECT CONVERT('123.4567', DECIMAL(10, 2)); -- 123.46<br/>
      SELECT CAST('123.4567' AS DECIMAL(10, 2)); -- 123.46<br/>
      SELECT CONVERT('2023-10-01', DATE); -- '2023-10-01'<br/>
      SELECT CAST('2023-10-01' AS DATE); -- '2023-10-01'<br/>
      SELECT CONVERT('01', DECIMAL); -- INT는 안됨<br/>
      SELECT CAST(003 AS CHAR); -- '3'<br/>
      ...
      <h1>DATE, TIME</h1>
      <pre>
      CURDATE(), CURRENT_DATE(), CURRENT_DATE; -- 현재 날짜 반환<br/>
      CURTIME(), CURRENT_TIME(), CURRENT_TIME; -- 현재 시간 반환<br/>
      NOW(), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP; -- 현재 날짜와 시간 반환<br/>
      ...
      <pre>DATE | TIME | YEAR | MONTH | DAY = DAYOFMONTH('2023-10-01 12:34:56')<br/>
        DATENAME | MONTHNAME | WEEKDAY ('2023-10-01 12:34:56')<br/>
        HOUR | MINUTE | SECOND('2023-10-01 12:34:56');<br/>
      -- 현재 날짜, 시간, 년, 월, 일, 시, 분, 초 반환<br/>
      DATE(NOW()) / TIME(NOW()) / YEAR(NOW()) / MONTH(NOW()) / DAY(NOW()) <br/>
      / HOUR(NOW()) / MINUTE(NOW()) / SECOND(NOW());<br/>
      ...
      <h3>차이 계산</h3>
      <pre>SELECT DATEDIFF('2023-10-01', '2023-09-01'); -- 30<br/>
      SELECT TIMESTAMPDIFF(DAY | MONTH | YEAR, '2023-10-01', '2002-09-01');<br/>

      SELECT name, birth, CURDATE(), TIMESTAMPDIFF(YEAR, birth, CURDATE()) AS age<br/>
      FROM employees WHERE birth IS NOT NULL;</pre>

      <h3>date_add; 날짜에 시간 더하기</h3>
      <pre>SELECT DATE_ADD | ADDDATE('2023-10-01', INTERVAL 1 DAY); -- '2023-10-02'<br/>
      SELECT DATE_ADD('2023-10-01', INTERVAL 1 MONTH); -- '2023-11-01'</pre>

      <h3>date_sub; 날짜에서 시간 빼기</h3>
      <pre>SELECT DATE_SUB | SUBDATE('2023-10-01', INTERVAL 1 DAY); -- '2023-09-30'<br/>
      SELECT DATE_SUB('2023-10-01', INTERVAL 1 MONTH); -- '2023-09-01'</pre>

      <h3>unix_timestamp; 현재 시간을 유닉스 타임스탬프로 반환</h3>
      <pre>SELECT UNIX_TIMESTAMP(); -- 현재 시간을 유닉스 타임스탬프로 반환<br/>
      SELECT UNIX_TIMESTAMP('2023-10-01 12:00:00'); -- 특정 시간을 유닉스 타임스탬프로 반환</pre>

      <h3>from_unixtime; 유닉스 타임스탬프를 날짜로 변환</h3>
      <pre>SELECT FROM_UNIXTIME(UNIX_TIMESTAMP()); -- 현재 시간을 날짜로 변환<br/>
      SELECT FROM_UNIXTIME(1696129200); -- 특정 유닉스 타임스탬프를 날짜로 변환</pre>

      <h3>utc_date, UTC_TIME ; UTC 반환</h3>
      <pre>SELECT UTC_DATE(); -- UTC 날짜 반환<br/>
      SELECT UTC_TIME(); -- UTC 시간 반환</pre>
    </div>
  )
}

export default PsqlFunctions
