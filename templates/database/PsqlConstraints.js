...
<pre>{`SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
  ...
  WHERE TABLE_SCHEMA = 'mydb';`}</pre>
    ...
    {/* <!-- @@@@@@@@@@@@@@ index @@@@@@@@@@@@@@@@@@@ --> */}
    <h3>INDEX; 특정 컬럼에 대한 검색 속도를 높임</h3>
    ...
    <h4>show indexes</h4>
  <pre>{`SHOW INDEXS FROM table_name\G; 
  -- index가 있는 column에 대한 정보를 보여줌`}</pre>

    <h4>create index; 테이블 생성시 적용</h4>
<pre>{`CREATE TABLE Customers (
  ...
  -- OR, CONSTRAINT IDX_CustomerName INDEX (CustomerName)
    ...
    <h4>CREATE INDEX로 추가; 중복 인데스 허용</h4>
...
<pre>{`CREATE INDEX idx_customer_name
  ...
  -- 하나씩 적용하면 seq_in_index가 1로 적용됨, 여러개 한번에 해야 우선순위를 갖음
...
<pre>{`CREATE UNIQUE INDEX index_name 
  ON table_name (column1, column2, ...);`}</pre>
    ...
    <h4>CREATE FULLTEXT INDEX</h4>
<pre>{`CREATE FULLTEXT INDEX index_name 
  ON table_name (column1, column2, ...);`}</pre>

    <h4>CREATE SPATIAL INDEX</h4>
...
<pre>{`CREATE TABLE Customers (
  ...
  -- OR, CONSTRAINT PK_Customer PRIMARY KEY (CustomerID, CustomerName)
    ...
    <h4>ALTER TABLE로 추가</h4>
<pre>{`ALTER TABLE Persons ADD PRIMARY KEY (ID);
ALTER TABLE Persons 
  ADD CONSTRAINT PK_Person PRIMARY KEY (ID, LastName);`}</pre>

    <h4>DROP a PRIMARY KEY Constraint</h4>
...
<pre>{`ALTER TABLE Orders ADD FOREIGN KEY (CustomerID)
  ...
  FOREIGN KEY (CustomerID) REFERENCES Customers(CustomerID);`}</pre>
    <h4>DROP a FOREIGN KEY Constraint</h4>
...
<pre>{`CREATE TABLE Persons (
  ...
  -- OR, CONSTRAINT CHK_Age CHECK (Age &gt;= 18 AND City = 'Sandnes')
    ...
    <h4>ALTER TABLE로 추가</h4>
<pre>{`ALTER TABLE Persons ADD CHECK (Age &gt;= 18);
  ALTER TABLE Persons ADD CONSTRAINT CHK_Age
...
<pre>ALTER TABLE Persons ALTER Age DROP DEFAULT;</pre>
    <h3>AUTO INCREMENT</h3><p>정수형 데이터에 대해 자동으로 증가하는 값을 생성</p>
    <h4>테이블 생성시 적용</h4>
<pre>{`CREATE TABLE Customers (
